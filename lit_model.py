...
for file_name in ['TohirMalik.txt', 'AlisherNavoiy.txt',
                  'OtkirXoshimov.txt', 'AbdullaQodiriy.txt', 'GofurGulom.txt']:
    with open(f'Writers_texts/{file_name}', 'r', encoding="utf-8") as f:
        file_text = f.read()
        file_text = file_text.replace('\n', ' ').split(' ')
        text_train.append(' '.join(file_text[:int(len(file_text) * 0.7)]))
...
def pred(file):
    ...
    for i in range(0, len(unique)):
        print(writers_list[unique[i]], counts[i])
